...
class Backend_Server:
    """
    The Class representing the Backend server which takes on the responsibility to both load and generate from the model.
    """
    def __init__(self):
        """
        Initializes the Backend_Server's data members and attaches API routers to their async functions.
        """
        self.tokenizer = None
        self.model = None
        self.device = None
    ...
    async def load(self, a_request: Request):
        """
        Loads the model as per requested from the front end.
    ...
    def replace_symbolic_data_type(self, a_model_settings: dict):
        """
        Replaces a string version of the data type of the model with its corresponding
        torch data type.

        :description: This function exists because there are some values we need use values known to the
        interpreter rather than in string format.
        ...
        :return: The updated model settings
        :rtype: dict
        """
    ...
    def load_model_settings(self, a_model_settings: dict):
        """
        Extracts the model settings from the parameter model_settings and puts it into the
    ...
    def initialize_experimental_warpers(self, a_experimental_warper_settings : dict):
        """
        Populates the experimental_warpers list data member with various
        ...
        :param a_experimental_warper_settings: The settings relating to experimental warper settings, which determine
        what experimental warpers will be added to the experimental_warpers list data member.
        :type a_experimental_warper_settings: dict
        """
    ...
    def initialize_experimental_processors(self, a_experimental_processor_settings:dict):
        """
        Populates the experimental_processor list data member with various
        ...
        surrounding serializing tuples, most of the oncoming processor data will be read as a 2d list.

        :algorithm: 1. Loop through each of the individual processors
        ...
        3. Gradually append the 'key' and 'values' to a list, which is then added to the experimental_processors
           list data member.
        :param a_experimental_processor_settings: The settings relating to experimental
        :type a_experimental_processor_settings: dict
    ...
    def dispatch_experimental_warpers(self, a_input_ids, a_generation_settings : dict):
        """
        Goes through each item of the experimental_warpers list data member
        ...
        :type a_input_ids: torch.Tensor, of varying type depending on data type.
        ...
        :type a_generation_settings: dict
    ...
    async def generate(self, a_request: Request):
        """
        Creates text given the writing prompt/input which is extracted from the user's server request.

        :param a_request: The user's request, which has several pieces of information critical to the model's text
        generation.
        :type a_request: Request

        :description: This function is the bread and butter of this program, the backbone, etc.. This function responds
        to the user requests to endpoint /generate, which is used the most often.

        :algorithm: 1. The request is retrieved, and the settings are each assigned to different variables.
...
def main():
    """
    The main driver of the back-end. Handles starting up the server on a certain port.
    """
